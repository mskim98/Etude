...
export function SATMockExams({ subjects, onStartExam, selectedSubject, onTabChange, className }: SATMockExamsProps) {
  // Mock SAT practice tests with enhanced data - Extended for scroll testing
  const satPracticeTests = [
    {
      ...
      timeSpent: 165 // minutes actually spent
    },
    {
      id: 'sat-test-2',
    ...
    {
      id: 'sat-test-13',
      title: 'SAT Subject Test - Advanced Math',
      completed: false,
      attempts: 0,
      duration: 120,
      scores: null,
    ...
    {
      id: 'sat-test-14',
      title: 'SAT Subject Test - Critical Reading',
      completed: false,
      attempts: 0,
      duration: 120,
      scores: null,
  ...
  const getDetailedStats = () => {
    ...
    const trend = completed.length > 1 ? 
      completed[completed.length - 1].scores!.total - completed[0].scores!.total : 0;
  ...
  return (
    <div className={`space-y-8 ${className}`}>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="text-center p-6 rounded-lg border" style={{ 
                  ...
                  <TrendingUp className="w-8 h-8 mx-auto mb-3" style={{ color: stats.trend >= 0 ? 'var(--color-success)' : 'var(--color-destructive)' }} />
                  <div className="text-2xl font-bold mb-1" style={{ color: 'var(--color-text-primary)' }}>
                  ...
                  <div className="text-2xl font-bold mb-1" style={{ color: 'var(--color-text-primary)' }}>
                    {(() => {
                      ...
                      const nearestExam = satSubjects
                        .map(s => ({ ...s, daysUntil: Math.ceil((s.examDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24)) }))
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-sm font-medium" style={{ color: 'var(--color-text-primary)' }}>Reading</span>
                    </div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-sm font-medium" style={{ color: 'var(--color-text-primary)' }}>Writing</span>
                    </div>
                    <span className="text-sm font-bold" style={{ color: 'var(--color-text-primary)' }}>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-sm font-medium" style={{ color: 'var(--color-text-primary)' }}>Math</span>
                    </div>
                    <span className="text-sm font-bold" style={{ color: 'var(--color-text-primary)' }}>
                      {stats.avgMath || 0}/800
                    </span>
      ...
      <Card className="border rounded-lg flex-1 flex flex-col h-full min-h-0" style={{ 
        backgroundColor: 'var(--color-card-default-bg)',
        borderColor: 'var(--color-card-border)',
        borderTop: '4px solid var(--color-accent)',
        boxShadow: 'var(--color-card-hover-shadow)',
            ...
            <Table style={{ tableLayout: 'fixed', width: '100%' }}>
              ...
              >
                ...
                >
                  ...
                  <TableHead className="text-base font-semibold py-4 text-center rounded-tr-lg" style={{ color: 'var(--color-text-primary)' }}>Action</TableHead>
                </TableRow>
              </TableHeader>
            </Table>
          </div>
                  ...
                  <TableRow 
                    ...
                    onMouseEnter={(e) => {
                      e.currentTarget.style.backgroundColor = test.completed 
                        ? 'rgba(0, 145, 179, 0.15)' 
                        : 'var(--color-muted)';
                      e.currentTarget.style.boxShadow = '0 8px 25px -5px rgba(0, 145, 179, 0.3), 0 10px 10px -5px rgba(0, 145, 179, 0.04)';
                    }}
                    onMouseLeave={(e) => {
                      e.currentTarget.style.backgroundColor = test.completed 
                  ...
                  >
                    <TableCell className="py-6">
                      <div className="flex items-center space-x-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="font-semibold text-lg truncate" style={{ color: 'var(--color-text-primary)' }}>
                            {test.title}
                          </div>
                    ...
                    <TableCell className="py-6">
                      <div className="text-center">
                        <div className="font-bold text-2xl" style={{ color: test.completed ? 'var(--color-accent)' : 'var(--color-text-tertiary)' }}>
                          {test.scores?.total || '--'}
                        </div>
                    ...
                    <TableCell className="py-6">
                      <div className="text-center">
                        <div className="font-semibold text-lg" style={{ color: test.percentile ? 'var(--color-text-primary)' : 'var(--color-text-tertiary)' }}>
                    ...
                    <TableCell className="py-6 text-center">
                      <Button
                        ...
                        onClick={() => {
                          // Find the subject by ID and pass it to onStartExam to navigate to SAT section select page
                          const subject = subjects.find(s => s.id === test.id);
                          if (subject) {
                            onStartExam(subject);
                          }
                        }}
