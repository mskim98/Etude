...
import type { Subject, ExamQuestion, ExamResult } from '../App';
...
export function MockExamPage({ subject, onExamComplete, onNavigate, onNavigateToSection }: MockExamPageProps) {
  ...
  const questions: ExamQuestion[] = [
    {
      ...
      correctAnswer: 3,
      explanation: 'The poem describes how the speaker, though physically tired, mentally "journeys" to think about their friend, keeping their "eyelids open wide" instead of falling asleep immediately.',
      subject: subject?.id || '',
      chapter: 'Reading and Writing'
    },
    {
      id: '2', 
      question: 'In a chemical reaction, the rate of reaction is directly proportional to the concentration of reactant A and the square of the concentration of reactant B.\n\nGiven information:\n• [A] = 0.1 M\n• [B] = 0.2 M  \n• Rate constant k = 5.0 L²/(mol²·s)\n\nThe rate law for this reaction is: Rate = k[A][B]²\n\nWhat is the initial rate of reaction?',
      options: [
        '0.02 M/s',
        '0.04 M/s', 
        '0.2 M/s',
    ...
    {
      id: '3',
      question: 'A quadratic function f(x) = ax² + bx + c has its vertex at (2, -3) and passes through the point (0, 1).\n\nUsing the vertex form of a quadratic function:\nf(x) = a(x - h)² + k\n\nwhere (h, k) is the vertex of the parabola.\n\nWhat is the value of coefficient a?',
      options: [
        'a = 1',
        'a = 2', 
        'a = -1',
      ...
      correctAnswer: 0,
      explanation: 'Using vertex form f(x) = a(x-h)² + k where vertex is (h,k) = (2,-3): f(x) = a(x-2)² - 3. Since f(0) = 1: 1 = a(0-2)² - 3 = 4a - 3, so 4a = 4, therefore a = 1.',
      subject: subject?.id || '',
      chapter: 'Algebra'
    }
  ];
          ...
          <div className="max-w-7xl mx-auto flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              >
                ...
                {subject?.type === 'AP' ? 'Back to AP Courses' : 
                 subject?.type === 'SAT' ? 'Back to SAT Exams' : 
                 'Back to Dashboard'}
              </Button>
            </div>
          </div>
          ...
          <Card className="text-center border-primary/20 shadow-xl bg-card">
            <CardContent className="p-12">
              ...
              <p className="text-muted-foreground mb-8 max-w-2xl mx-auto">
                ...
                You'll have 3 hours to complete {questions.length} questions in a BlueBook-style interface with access to tools and formulas.
              </p>
              ...
              <Alert className="mb-8 bg-primary/5 border-primary/20 shadow-sm">
                ...
                <AlertDescription className="text-foreground">
                  <strong className="text-primary">Tools Available:</strong> You'll have access to a calculator, notepad for scratch work, common formulas, and can flag questions for review. 
                  Navigate freely between questions during the exam.
                </AlertDescription>
              </Alert>

              <Button size="lg" onClick={() => setExamStarted(true)} className="bg-primary text-primary-foreground hover:bg-primary/90 shadow-md px-8 py-3 text-lg font-semibold">
                Start Exam
              </Button>
            </CardContent>
          </Card>
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <header 
        className="bg-white px-6 py-3 flex-shrink-0"
      ...
      <div className="flex-1 flex relative">
                ...
                {questions.map((_, index) => {
                  ...
                  return (
                    <Button
                      ...
                      style={{ 
                        backgroundColor: isCurrent ? 'var(--color-primary)' : isAnswered ? 'var(--primary-light)' : 'transparent',
                    ...
                    >
                      {index + 1}
                      {isFlagged && (
                        <Flag className="w-3 h-3 absolute -top-1 -right-1 text-red-500 fill-current" />
                      )}
                    </Button>
                  );
                })}
        ...
        <div className="flex-1 flex">
          ...
          <div className="w-1/2 bg-white flex flex-col">
            {/* Question Header */}
            <div className="border-b px-6 py-4" style={{ backgroundColor: 'var(--color-primary-light)', borderColor: 'var(--color-primary)' }}>
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <Button
                    ...
                    style={{
                      ...
                      backgroundColor: flaggedQuestions.has(currentQuestion) 
                        ? 'rgba(0, 145, 179, 0.1)' 
                        : 'transparent'
                    }}
                  >
            ...
            {/* Question Text */}
            <div className="p-6 border-b" style={{ borderColor: 'var(--color-primary)', backgroundColor: 'var(--color-primary-light)' }}>
              <div className="prose max-w-none">
                <div className="whitespace-pre-wrap text-base leading-relaxed" style={{ color: 'var(--color-text-primary)' }}>
                  {questionText}
                </div>
              </div>
            </div>
                    ...
                    <div className="flex items-start space-x-3">
                      <div className="mt-0.5">
                        {answers[currentQuestion] === index ? (
                          ...
                          <div className="w-5 h-5 border-2 rounded-full" style={{ borderColor: 'var(--primary)' }}></div>
                        )}
                      </div>
                      <div className="flex-1">
                        ...
                        <span className="text-base" style={{ color: 'var(--text-primary)' }}>{option}</span>
                      </div>
                    </div>
                  </div>
                ))}
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center justify-center space-x-4">
            ...
            <Button
              ...
              style={{ 
                backgroundColor: currentQuestion === questions.length - 1 ? 'var(--color-text-tertiary)' : 'var(--color-primary)',
