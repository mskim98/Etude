...
export function SATSectionSelectPage({
		...
		const baseSections = [
			{
				...
				name: "Reading",
				description:
					"Reading comprehension, vocabulary in context, and analysis of texts from literature, history, and science.",
				duration: 65,
				questions: 52,
				icon: BookOpen,
				color: "#0091B3",
	...
	const overallAccuracy =
		totalCompletedQuestions > 0 ? Math.round((totalCorrectAnswers / totalCompletedQuestions) * 100) : null;

	const getDifficultyColor = (difficulty: string) => {
		switch (difficulty.toLowerCase()) {
			case "easy":
				return "var(--color-difficulty-easy)";
			case "medium":
		...
		<div className="min-h-screen" style={{ backgroundColor: "var(--background)" }}>
			...
			<div className="max-w-7xl mx-auto px-6 py-8">
				...
				<Card
					...
					style={{
						backgroundColor: allSectionsCompleted ? "var(--color-card-completed-bg)" : "var(--color-card-default-bg)",
						borderColor: allSectionsCompleted ? "var(--color-status-success)" : "var(--color-card-border)",
						borderLeft: allSectionsCompleted
					...
					<CardContent className="py-4">
						<div className="flex items-center justify-between">
							<div className="flex-1">
								...
								<div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
									...
									>
										<div
											...
											style={{
												color: allSectionsCompleted ? "white" : "var(--color-text-secondary)",
											}}
										>
											{allSectionsCompleted ? totalScore : "---"}
										</div>
										<div
											className="text-sm"
											style={{
									...
									<div
										...
										style={{
											borderColor: "var(--color-card-border)",
											backgroundColor: allSectionsCompleted ? "var(--color-accent)" : "var(--color-card-default-bg)",
										}}
									>
										<div
											className="text-2xl font-bold mb-1"
										...
										>
											{allSectionsCompleted ? `${overallAccuracy}%` : "---%"}
										</div>
										<div
											className="text-sm"
											style={{
									...
									{sections.map((section) => (
										...
										>
											...
											<div className="text-sm" style={{ color: "var(--color-text-tertiary)" }}>
												{section.completed && section.correctAnswers
													? `${section.correctAnswers}/${section.questions}`
													: "---"}
											</div>
										</div>
									))}
								...
								<div className="flex items-center text-sm mt-3" style={{ color: "var(--color-text-secondary)" }}>
									<Target className="w-4 h-4 mr-1" />
									<span>{allSectionsCompleted ? "All sections completed" : "Complete sections to see results"}</span>
								</div>
							</div>
							...
							<div className="ml-4">
								<Button
									...
									style={{
										backgroundColor: allSectionsCompleted ? "var(--color-accent)" : "var(--color-text-tertiary)",
										color: "white",
										border: "none",
