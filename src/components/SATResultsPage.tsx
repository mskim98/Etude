...
import { SectionScoreCard } from "@/components/results/SectionScoreCard";
import { TypeAnalysisChart } from "@/components/results/TypeAnalysisChart";
import { WrongQuestionList } from "@/components/results/WrongQuestionList";
import { ConversionReferenceCard } from "@/components/results/ConversionReferenceCard";
...
export function SATResultsPage({
  result,
  subject,
  onNavigate,
  onRetryExam
  ...
  const getCurrentSectionData = () => {
    switch (activeSection) {
      case "reading": return readingData;
      case "writing": return writingData;
      case "math": return mathData;
      default: return readingData;
    }
  };
  ...
  const getSectionScore = () => {
    switch (activeSection) {
      case "reading": return Math.round(readingWritingScore * 0.5);
      case "writing": return Math.round(readingWritingScore * 0.5);
        ...
        <div className="mb-8">
          <Tabs value={activeSection} onValueChange={(value) => setActiveSection(value as SATSection)}>
            ...
            <TabsContent value={activeSection}>
              ...
              <div className="mb-8">
                <ConversionReferenceCard section={activeSection.charAt(0).toUpperCase() + activeSection.slice(1) as any} />
              </div>

              {/* Section-specific Question Type Analysis */}
              <div className="mb-8">
                <TypeAnalysisChart
                  title={`${activeSection.charAt(0).toUpperCase() + activeSection.slice(1)} Performance by Question Type`}
              ...
              <div className="mb-8">
                <WrongQuestionList
                  questions={getSectionWrongQuestions()}
