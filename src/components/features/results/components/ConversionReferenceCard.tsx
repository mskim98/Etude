			...
			<Collapsible open={isOpen} onOpenChange={setIsOpen}>
				<CollapsibleTrigger asChild>
					<CardHeader className="cursor-pointer hover:bg-gray-50 transition-colors">
						...
						<p className="text-xs text-gray-500 text-left">Raw score to scaled score reference table</p>
					</CardHeader>
				</CollapsibleTrigger>
				...
				<CollapsibleContent>
					<CardContent>
						<div className="max-h-64 overflow-y-auto">
							<Table>
								...
								<TableBody>
									{data.map((row, index) => (
										<TableRow key={index}>
											<TableCell className="text-xs font-medium">{row.raw}</TableCell>
						...
						<div className="mt-3 p-2 bg-gray-50 rounded text-xs text-gray-600">
							<strong>Note:</strong> These are approximate conversions. Actual SAT scoring may vary based on test
							difficulty and equating.
						</div>
					</CardContent>
				</CollapsibleContent>
			</Collapsible>
